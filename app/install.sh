...
get_latest_release() {
  fetch "https://api.github.com/repos/camflan/markdown-preview.nvim/releases/latest" |
    grep '"tag_name":' |
    sed -E 's/.*"([^"]+)".*/\1/'
}
...
download() {
  mkdir -p bin
  cd bin
  url="https://github.com/camflan/markdown-preview.nvim/releases/download/$tag/${1}"
